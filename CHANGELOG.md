...
### 涉及编号变更
- 版本：`1.51.1+162` → `1.51.2+163`（焦点卡间距与课程名字号调整；仅布局资源变更）。
...
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| UI | **首页删除占位「+」FAB**：无功能空回调入口移除(右下角),与待办/工具页"不留占位按钮"约定统一;回归测试改断言"无 FAB" | P-01-01 / C-24 | Android 11+ / Web |
| 功能 | **首页 2×1 仪表盘卡 →「待办」实时卡**：标题更名「待办」;中间三段真实统计(今日完成/总任务/当前待办);底部四段进度(已完成·已逾期·今天到期·未来,权重=数量,语义色 primary/error/secondary/outline,空数据弱化占位);点击卡片跳待办一级页(/?page=0) | C-29 / C-34 | Android 11+ / Web |
| 数据 | **todoOverviewProvider 派生统计**：watch 待办全量+归档,口径=总任务(items 全量)/待办(未完成)/今日完成(archived completedAt 当天)/逾期·今天·未来按截止日分组;待办页增删改/勾完成后返回首页自动刷新(纯函数可单测) | S-23 / C-29 | Android 11+ / Web |
| 修复 | 待办卡点击跳转失效：shell 补 routerDelegate 运行时监听(外部 go('/?page=N') 统一同步翻页,内部翻页幂等跳过) | P-01 / C-29 | Android 11+ / Web |
| 测试 | +todoOverviewProvider 口径单测(分组/跨日边界/空数据;共 126,analyze 0) | — | — |

### 涉及编号变更
- 版本：`1.48.1+151` → `1.49.0+152`(首页待办卡联动批,152 为真机验证构建占用)。

## v1.48.1（2026-09-07）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| UI | **编辑页 MIUI 无底化**:悬浮工具 FAB 删除毛玻璃圆底/阴影 → 纯图标+圆形按压遮罩(C-21 返回键同语言),44 热区/19 图标,抬高 right14·bottom30,缩略图避让同步 | C-48 / P-11 | Android 11+ / Web |
| UI | **工具箱面板 MIUI 化**:圆角 22/边框淡化;节点添加按钮语义色点+文字(开始主色/步骤灰/判断次级/结束错误);泳道/操作/播放/导出图标化(undo/redo/delete/copy/paste/play/share/gridView);文字 onSurface 语义色;C48V2 去 Consumer 化(摘除 platform/模糊依赖) | C-48 | Android 11+ / Web |
| UI | **详情弹层行 icon 化**:锁定 lock/unlock 图标、编辑尺寸 edit 图标 | P-11 | Android 11+ / Web |
| 功能 | **播放连线流光动画**:激活线 GradientFlowEffect(独立 animated 渲染层,静态层不重绘;换线/停止/结束清除,静止零 ticker 恢复) | P-11 / vyuh vendor | Android 11+ / Web |
| UI | **宽屏右栏可调宽**:画布与详情栏间拖拽分隔条(240-420,resize 光标,拖拽高亮) | P-11 | Android 11+ / Web |
| UI | **画布 hover 细化(vendor v0.5.0)**:节点悬停描边主色+微阴影、泳道虚线转实线;连线悬停高亮色+1px(快照 hoveredIds 注入,选中优先,移出画布清理);触摸零开销 | P-11 / vyuh vendor | Web / 桌面 |

### 涉及编号变更
- 版本：`1.48.0+150` → `1.48.1+151`(收尾批:播放动画/右栏调宽/hover/MIUI UI,151 为收尾归档构建占用)。
- 前端内核补丁：`third_party/vyuh_node_flow` hover 体系(node.isHovered / controller.hoveredNodeId·hoveredConnectionId / 快照 hoveredIds / 画布退出清理,注释锚点 `POC vendor 补丁(v0.5.0)`);动画走库内建 Connection.animationEffect(零补丁)。
- 流程总结：见 `REPORT_flow_editor_vyuh_v1.48.md`(全链路成果/决策/遗留/开源同步待办)。

## v1.48.0（2026-09-07）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| UI | **竖屏工具栏 C-48 悬浮收纳 v2**：底部 18 按钮横排工具行 → 右下悬浮毛玻璃圆钮(U-03 模糊政策,展开 ✕/收起画笔)展开能力面板(节点 2×2/泳道/撤销/重做/删除选中/复制/粘贴/适应/缩略图/简化/图例(开关态高亮)/播放/导出;动作后自动收起;点画布空白收起;播放中与尺寸编辑模式隐藏;缩略图抬升避让) | C-48 / P-11 | Android 11+ / Web |
| UI | **宽屏/横屏三段式(≥700px)**：左 C-48 docked 常驻卡 · 中画布 · 右详情栏 272(点选节点/连线右侧查看编辑,空态提示,画布空白点击关闭);窄屏保持 FAB+弹层;播放条/尺寸条双形态(窄屏底部通栏/宽屏顶部悬浮胶囊) | P-11 / C-48 | Android 11+ / Web |
| 功能 | **自动平移(AutoPan)**：拖动节点/连线到画布边缘 50px 自动滚动(拖长图不松手) | P-11 / vyuh vendor | Android 11+ / Web |
| 功能 | **PC/Web 键盘全集**：Ctrl+Z 撤销 / Ctrl+Shift+Z·Ctrl+Y 重做 / Ctrl+C 复制 / Ctrl+V 粘贴 / Ctrl+X 剪切(宿主接管,vyuh 内建 copy/paste 未实现);Delete/Backspace 删除(内建带锁检查)、Ctrl+A 全选、滚轮缩放保留内建 | P-11 | Web / 桌面 |
| 性能 | 审计:静止零 ticker(默认无连线动画特效,动画仅交互/播放事件驱动)、深浅/Monet 换肤热更新不丢视口;双指缩放"空白起手"评估:节点上第二指=画布平移、空白双指=缩放(行为自洽,保留现状);百节点流畅度留真机验收 | P-11 / vyuh vendor | Android 11+ / Web |
| 测试 | 触摸 resize 双路径 widget 测试归档(库把手/overlay 把手;共 123,analyze 0) | — | — |

### 涉及编号变更
- 版本：`1.47.0+149` → `1.48.0+150`(阶段 3 响应式与键盘;150 为阶段内真机验证构建占用)。
- 规划文档：PLAN_v1.46_flow_vyuh.md §8(阶段 3 落地:竖屏收纳/三段式/键盘/autoPan;性能审计结论如上;PC 中键平移随 InteractiveViewer 鼠标事件,桌面验证后如有缺口再补)。
- 评估后不做(后续版本)：播放连线生长/流动动画增强、触摸双指缩放视口手势自研、C-48 右栏可调宽与 hover 态细化。

## v1.47.0（2026-09-07）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 功能 | **泳道 LaneNode(阶段 2)**:新增「＋泳道」工具;vyuh GroupNode 空间包含语义(虚线圆角分区+标题 chip 自绘/节点拖入拖出自动归属/随泳道拖动整体移动/可拉伸/自定义色/深浅与 Monet 跟随);FlowDoc v2 节点新增 lane 语义与 w/h 尺寸覆盖(拉伸结果落盘);旧数据 lane 元素迁移为 LaneNode(标题/尺寸保留,不再跳过) | P-11 / vyuh vendor | Android 11+ / Web |
| 功能 | **连接校验**:拉线起点/落点接入 canConnect 链(锁定节点禁线/防自环/同对去重/判断出口唯一,toast 提示;规则纯函数可测) | P-11 | Android 11+ / Web |
| 功能 | **批量操作**:复制选中子图(流程节点+内部连线,端口按出入度重编)/粘贴到视口中心(新 id+平移+粘贴组选中);删除选中/粘贴走批量边界(撤销一步回退整批,BatchEnded 单次入栈) | P-11 | Android 11+ / Web |
| 功能 | **锁定链路补全**:锁定节点禁拖动(库)/禁拉伸(补丁)/禁删除/禁拉线;详情弹层锁定中禁改(仅可解锁开关);删除选中跳过锁定项 | P-11 / vyuh vendor | Android 11+ / Web |
| 功能 | **视图增强**:右下缩略图(Miuix 实时主题,点击/拖动导航)、LOD 细节层次开关(缩小时隐藏细节,大图性能)、图例弹层(五类语义+连线说明);画布深浅/Monet 换肤热更新链路(editor updateTheme,不丢视口) | P-11 / vyuh vendor | Android 11+ / Web |
| 数据 | FlowDoc v2 节点 `w/h`(仅泳道写盘;非泳道恒缺省,旧文档兼容);HTML 导出 v2 泳道虚线分区(w/h/标题) | P-11 / D-01 | Android 11+ / Web |
| 测试 | +泳道模型/迁移/codec/导出、校验规则、剪贴板子图单测(共 121,analyze 0) | — | — |

### 涉及编号变更
- 版本：`1.46.0+148` → `1.47.0+149`(阶段 2 能力补全完成)。
- 前端内核补丁：`third_party/vyuh_node_flow` node_container 锁定禁拉伸(`POC vendor 补丁(v0.3.0 阶段2)` 注释锚点)。
- 规划文档：PLAN_v1.46_flow_vyuh.md §7(阶段 2 全部落地;LaneNode 采用 GroupBehavior.bounds 空间包含语义)。
- 评估后不做(后续版本)：阶段 3 —— C-48 工具栏收纳与三段式布局适配新内核、PC 键盘全集/滚轮缩放、播放连线动画增强、性能审计(百节点 60fps/静止零占用/深浅无闪烁全矩阵回归)。

## v1.46.0（2026-09-06）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 内核 | **流程图编辑器内核迁移 vyuh_node_flow 0.32(阶段 1 完成)**:旧 flutter_flow_chart 内核移除;新 P-11 v2(/todo/:taskId)——FlowDoc(v2)持久化 + 旧数据自动迁移写回;拖动/连线/框选/键盘内建;单击节点/连线详情与样式面板(改名/批注/锁定/自定义色/删除;线分支名/色/粗细/箭头三态);撤销/重做(整图快照栈 20,即时入栈);逻辑播放(开始起沿出线推进/判断分支选路/自动三档速度/单步/播放中禁改);HTML 播放文件导出(v2 适配,旧输入兼容);触摸体验(POC 移植:手动拖动全局指针路由/单击抬手判定/端口按下即拉线/长按=右键/双指平移/命中热区) | P-11 / vyuh vendor | Android 11+ / Web |
| 数据 | 流程图落盘 flowchart_$id 改 FlowDoc(v=2);旧 Dashboard JSON 打开即迁移写回;泳道(lane)元素迁移暂时跳过(阶段 2 LaneNode) | P-11 / S-23 | Android 11+ / Web |
| 测试 | +迁移器/模型/codec/导出器 v2 单测(共 104,analyze 0) | — | — |

### 涉及编号变更
- 版本:`1.45.0+141` → `1.46.0+148`(内核替换+功能多批;142-147 为阶段内真机验证构建,只升不降)。
- 依赖:flutter_flow_chart(third_party 移除)换 vyuh_node_flow 0.32.0(third_party,含 POC 触摸/渲染补丁,注释锚点 `POC`);+flutter_mobx。
- 规划文档:PLAN_v1.46_flow_vyuh.md(阶段 0 结论/补丁清单/阶段 1-3 蓝图;POC 工程 `_poc_vyuh` 保留参考)。
- 评估后不做(后续版本):泳道 LaneNode(阶段 2)、连接校验规则/批量操作/Minimap/LOD、C-48 工具栏收纳与三段式布局/PC 键盘全集(阶段 3)、播放连线流动/生长动画增强。

## v1.45.0（2026-09-06）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 功能 | **流程图编辑器批E（宽屏三段式）**：P-11 横屏/阔屏三列布局(左任务列/中画布/右上下文) + C-48 工具栏 docked;竖屏维持悬浮收纳 | P-11 / C-48 | Android 11+ / Web |
| UI | **顶栏回官方 flutter_miuix 方案**：C-25 折叠大标题顶栏重写为官方 `MiuixTopAppBar(blurred: 按 U-03 生效)`(删除自绘渐变蒙版/RepaintBoundary,折叠联动不变,11 页统一);P-02 调色板恢复官方折叠标题;P-11 顶栏改官方 `MiuixSmallTopAppBar` | C-25 / P-02 / P-11 | Android 11+ / Web |
| UI | **MIUI 无底化按钮**：C-21 返回胶囊改官方 MiuixPressable 遮罩反馈纯图标(全部二级页左上角,热区 40,摘除死代码 collapseState);C-43 日期左右箭头去圆形底色(与月历头部统一) | C-21 / C-43 | Android 11+ / Web |
| UI | **连线生长点亮动画时长 900→1200ms**：与自动播放默认步进 1:1 咬合(第三方内核 vendor 调参) | P-11 | Android 11+ / Web |
| UI | **卡片阴影与暗色高光统一**：CardShadow 内建暗色高光(深色 1px 白描边+微光晕,浅色零开销)——首页摘要/网格/课表/工具入口等全部 CardShadow 卡自动获得;C-44 待办任务卡与 P-12 回收站条目外包 CardShadow(首页同款周围阴影);C-42/C-37 旧嵌套描边撤除防双重 | C-44 / P-12 / cards | Android 11+ / Web |
| 功能 | **原生式开屏(双段)**：Android 12+ 系统 Splash(values-v31/night,应用图标+品牌底色)+ C-50 改真实应用 logo(与 launcher 同图)、时长 700→1100ms、缩放淡入原生节奏、去文字;支持后台复位后回前台重播 | C-50 / android | Android 11+ / Web |
| 功能 | **后台最小化(S-24)**：进入后台 ≥15s → 广播复位(浮层收拢/日历/编辑态退出/编辑器落盘)→ pop 二级页 → 回默认首页;回前台重播开屏;15s 内返回原样(计时+时间戳双保险,引擎冻结也能补判) | S-24 / shell | Android 11+ / Web |
| 性能 | 按钮无底化摘除 C-21 折叠阈值监听(Stateless 化);卡片高光浅色透传零开销 | C-21 / cards | Android 11+ / Web |

### 涉及编号变更
- 版本：`1.44.0+138`(HEAD 07c4250 之后工作区未归档批)→ `1.45.0+141`(批E+UI 多批+开屏+后台,只升不降;139/140 为真机验证构建占用)。
- 前端内核调参：`third_party/flutter_flow_chart` draw_arrow 生长点亮 900→1200ms(vendor 内注释标记)。
- 评估后不做(后续版本)：流程图编辑器内核选型评估 —— vyuh_node_flow 0.32 POC 已验收(独立工程 `_poc_vyuh`,阶段 0 报告含 vendor 补丁清单与 C=dynamic 泛型结论);v1.46 起阶段 1 内核替换。

## v1.43.0（2026-09-05）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 功能 | **待办一级页（P-10，底栏最左「待办」）**：单日视图 —— C-43 日期导航(左右切日 + 自绘月历:PageView 侧滑切月/年份选择/今天描边)切换日期,仅显示当日未完成;空态引导 | P-10 / C-43 | Android 11+ / Web |
| 功能 | **任务 CRUD**：C-44 任务卡(优先级色点/副行/完成✓圆钮);FAB(毛玻璃与首页统一)新建/点按编辑 bottom sheet(标题/截止日期/优先级三档低中高);✓完成即归档 | C-44 / S-23 | Android 11+ / Web |
| 功能 | **回收站（P-12 /todo/archived，右上入口）**：已完成任务精简条目(原截止日/完成日);恢复回待办(流程图数据保留);永久删除(确认,连带流程图);过期清理(启动扫描:已完成超「截止日次日+15天」删流程图) | P-12 / S-23 | Android 11+ / Web |
| UI | 待办页左上角不再放占位假按钮;顶栏回收站入口为裸图标;FAB 毛玻璃化与首页 C-24 统一 | P-10 | Android 11+ / Web |
| 修复 | shell 窄/宽布局切换(旋转)重建 PageView 丢页 → 帧后校正回当前页;悬浮胶囊指示器冷启动错位(默认落首页时停在待办) → initState 直接落位;P-10 内容 Stack 误用 Positioned.fill 致整页 0 尺寸(白屏) | shell / C-22 / P-10 | Android 11+ / Web |

### 涉及编号变更
- 版本：`1.42.0+136` → `1.43.0+137`(新一级页+二级页+组件+服务 → Minor,§1.3;只升不降)。
- 数据 key：`todo.items` / `todo.archived` / `flowchart_$todoId`(流程图 v1.44.0 写入,清理逻辑已就位)。
- 路由：底栏三页映射 /todo→page=0 /home→page=1 /tools→page=2;新增顶层 R-15 `/todo/archived`。
- 评估后不做(后续版本)：v1.44.0 流程图编辑器(选型 flutter_flow_chart 4.1.1) + 导出(15 天窗口) + 横屏三栏预览 + 待办通知提醒。

## v1.42.0（2026-09-05）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 功能 | **课程表起止周编辑**：周次四选升级 —— 每周/单周/双周外新增「**指定起止周**」(开始周-结束周 1..30,起 ≤ 止校验,如第 2-18 周);保存写入具体周次(weeks),课表网格/首页迷你课表/课程提醒按周精确显示;编辑导入课(weeks 非空)自动回显起止周,不再伪装「每周」防静默丢数据;课程块新增周次角标(第 2-18 周) | P-06 / S-15 | Android 11+ / Web |
| 修复 | **工具参数选中胶囊同色遮挡**：C-40 select 选中态主色底+主色文字不可读 → 改浅主色底(14%)+主色文字+主色细边(与课表/设置选中语言统一) | C-40 / P-09 | Android 11+ / Web |
| 修复 | **带参工具「参数无效」提示完善**：UAPI 鉴权改 `Authorization: Bearer` 头(query `key=` 实测被拒 401,配置过 key 即全请求失败);纯文本参数留空本地拦截提示(不再空参撞 400);400 错误卡附**参数格式参考**(参数名+placeholder 示例,如 GitHub 仓库需 owner/repo 完整格式) | T-03 / P-09 / C-39 | Android 11+ / Web |
| 性能 | **摘除页面级快照采样管线(滚动掉帧/功耗主因)**：C-25 顶栏/C-26 菜单已改纯蒙版不再消费快照,但 10+ 页面仍各自挂 C-28 降采样+CaptureHeartbeat —— 滚动中每 2-4 帧整屏 toImageSync 白算(一级页双重采样) → 全部拆除,仅保留 shell 层唯一采样源(悬浮胶囊);滚动帧率与功耗直接受益 | C-28 / C-22 / P-01-01 等全页面 | Android 11+ / Web |

### 涉及编号变更
- 版本：`1.41.0+135` → `1.42.0+136`(功能+修复+性能 → Minor,§1.3;只升不降)。
- 评估后不做(④ B/C/D 后续项)：滚动中卡片阴影降档(视觉微变,待体感不足再上)、工具页互斥展开/单订阅减负、shell 二级页采样停摆(已随页面层拆除大幅缓解)。

## v1.41.0（2026-09-05）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 功能 | **UAPI 工具目录扩展 C 批（+7，总数 69）—— 图片上传工具箱**：图片压缩(级别/格式可选) · OCR 文字识别(全文提取) · 图片抠图(去背景,PNG 结果) · 图片敏感检测(评分/风险中文映射) · 图片隐水印嵌入(短内容 ≤9 字符) · 水印提取 · 表情包生成(配文图) | P-01-04 / F-04 / P-09 | Android 11+ / Web |
| 增强 | **file 参数全链路**：C-40 渲染「选择图片」卡(file_picker 相册/文件,缩略预览/更换)；提交**强制校验**未选图拦截；ToolApiService **multipart/form-data** 上传(文本参数按归属进 query/fields,文件魔数补扩展名);网络层重试同样覆盖 multipart | C-40 / T-03 / P-09 | Android 11+ / Web |
| 增强 | **结果图支持 base64 两态**：抠图/隐水印返回纯 base64 PNG → C-41 魔数校验解码渲染(Data URI 与纯 base64 均可),长按保存直接落盘 | C-41 | Android 11+ / Web |
| 测试 | +multipart 上传单测(自定义 BaseClient 捕获原始字节流断言),80 项全绿 | — | - |

### 涉及编号变更
- 版本：`1.40.1+134` → `1.41.0+135`(功能 → Minor,§1.3;只升不降)。
- 数据：`assets/tools/tools.json` 62 → 69 条(图片分类 +7)。
- 评估后不做：图片解码缩放(RGB565 等嵌入式格式,用途窄)、Base64 图床(需外部 data URI)、SVG 转图(相册无 SVG)、AIGC 标识(需 27 位服务商编码,面向 B 端)。

## v1.40.1（2026-09-05）[Android] [Web]

### 变更清单
| 变更类型 | 变更说明 | 涉及编号 | 平台兼容性 |
| :--- | :--- | :--- | :--- |
| 修复 | **课程常驻通知不再依赖 App 进程（C 方案）**：此前「点通知进应用 → 到点通知消失后常驻不出现」—— 常驻启动押在 Dart provider 时间刷新上，后台/恢复场景无重算事件即缺失。现**闹钟到点广播原生直启倒计时前台服务**：排课程闹钟时把结束时刻/总时长/结束文案写入闹钟 extra 与持久化清单（开机重排同恢复）→ Receiver 弹到点通知后立即启动常驻（endAt 校验 + Android 12+ 后台 FGS 豁免异常静默）；**闹钟能响，常驻必然出现，App 被杀/后台也成立**；下课 endAt 到服务原生自动停 | P-01-01 内部件 / C 桥 | Android 11+ |
| 测试 | 开发者弹层「10 秒后弹到点提醒」升级为**完整联测**：携带常驻参数（到点 + 2 分钟自治倒计时）—— 排程后可杀进程验证「闹钟拉起 → 常驻出现」 | P-01-02 | Android 11+ |

### 涉及编号变更
- 版本：`1.40.0+133` → `1.40.1+134`(修复 → Patch,§1.3;只升不降)。
- 闹钟持久化条目新增 endAtMillis/totalMinutes/endText（旧条目兼容：重排不带常驻参数，App 打开后重排即升级）。
- 已知边界：精确闹钟/闹钟档豁免外的降级链(普通 set)被系统拦截后台 FGS 时，常驻由 App 打开后桥按当前上课态重建（本版未做前台恢复校正，待后续如需再补）。
